...
int main()
{
	...
	Texture2D bg = LoadTexture("ground.png");
	bg.height = screenheight;
	bg.width = screenwidth;

	Texture2D bg2nd = LoadTexture("ground2.png");
	bg2nd.height = screenheight;
	bg2nd.width = screenwidth;
	...
	while (!WindowShouldClose())
	{
		...
		{
			DrawTexture(bg, bg1, 0, WHITE);
			DrawTexture(bg2nd, bg2, 0, WHITE);
		}
		else if (level == 3)
		{
		...
		for (int i = 0; i < max_enemies - enemies; i++)
		{
			if (Enemies[i].isAlive)
			{
				...
				Enemies[i].x -= (enemyspeed + playerSpeed);
		...
		if (IsKeyDown(KEY_RIGHT))
		{
			...
			fliped = false;
			playerSpeed = backgroundmovement;
		}
		...
		if (Score != 0 && Score % 10 == 0 && !levelUpScreen)
		{
			levelUpScreen = true;
		...
		DrawText(TextFormat("Score: %d", Score), 10, 10, 20, GOLD);
		DrawText(TextFormat("Health: %d", Health), screenwidth - 150, 10, 20, GOLD);
		DrawText(TextFormat("Level: %d", level),screenwidth/2-50, 10, 20, GOLD);
